...
### Added — Linux support
- **Graphical installer (`linux-installer/`)** — the Linux equivalent of the Windows `setup.exe`: a tiny (~1 MB) AppImage users download and double-click. A GTK setup window (zenity) detects the GPU, downloads the matching self-contained engine (CPU or NVIDIA GPU) with a progress bar, adds an applications-menu entry and launches StemTube. The engine runs with `--appimage-extract-and-run`, so **no `libfuse2` and no `sudo`** — nothing is installed system-wide. Falls back to text mode when run headless.
